...
`_build_user_message(email, rep_type=None)` formats the email's From, To, Subject, Date, and Body fields into a prompt string. When `rep_type` is provided, a "Rep role: {type}" line is prepended. Body text is truncated to 4000 characters. For follow-up emails, a "Previous conversation" section with chain context is prepended.
...
| `GET /feed` | Alias for `GET /` (Inbox). Kept for backwards compatibility. |
...
- `run_rescore_job(session, job_id)` — deletes email scores in committed batches of 100 to bound crash exposure, then deletes all chain_scores, then calls `score_unscored_emails` to re-score all emails and chains. Chain scores are deleted after email scores so that a crash during email score deletion does not leave chain scores permanently lost. Result summary includes `scored`, `errors`, `tokens`, `chains_scored`, `chain_errors`.
