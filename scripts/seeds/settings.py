...
SETTINGS_SEED = {
    ...
    "classifier_prompt_blocks": {
        "opening": "You are an email classification assistant. Analyse the following email and determine its type and extract any quoted/forwarded email metadata.",
        "email_type": '**email_type** — Classify as one of: "real_email" (genuine human correspondence), "auto_reply" (out-of-office, automatic reply, vacation responder), "bounce" (delivery failure, undeliverable), "calendar" (meeting acceptance, decline, tentative), "newsletter" (bulk/marketing email). Default to "real_email" if uncertain.',
        ...
        "closing": 'Respond with ONLY a JSON object in this exact format, no other text:\n{\n  "email_type": "<one of: real_email, auto_reply, bounce, calendar, newsletter>",\n  "quoted_emails": [{"from_email": "<email>", "subject": "<subject>", "date": "<date or null>"}]\n}',
    },
